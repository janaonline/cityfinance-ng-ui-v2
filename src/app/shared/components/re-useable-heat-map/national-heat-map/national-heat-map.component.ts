...
import { RupeeConverterPipe } from "../../../../core/pipes/rupee-converter.pipe";
...
export class NationalHeatMapComponent implements OnChanges, OnDestroy {
  ...
  ulbsSelected: any = new FormControl([]);
  ...
  mouseHoverOnState!: IStateULBCovered | null;
  ...
  districtMap!: L.Map | null;

  currentULBClicked!: ULBWithMapData | null;
  ...
  selectULBById(ulbId: string) {
    ...
    const marker: any = this.getDistrictMarkerOfULB(ulbFound);
  ...
  listenToFormControls() {
    this.ulbsSelected.valueChanges.subscribe((newValue: any) => {
  ...
  filterMergedStateDataBy(options: { ulbName?: string; stateId?: string | null }) {
  ...
  filteredULBBy(options: { ulbName?: string }, ulbList: ULBWithMapData[]) {
    ...
    if (options.ulbName && options.ulbName.trim()) {
      filteredULBS = filteredULBS.concat(
        ulbList.filter((ulb) =>
          ulb.name.toLowerCase().includes(options.ulbName ? options.ulbName.toLowerCase() : '')
        )
      );
    } else {
  ...
  mouseOut(stateLayer: any) {
    console.log("haid...");
    stateLayer.eachLayer(function (layer: any) {
      layer.setStyle({
        fillColor: "#3E5DB1",
        fillOpacity: 1,
      });
  ...
  createTooltip(layer: any, stateLayer: any) {
    if (this.isMapOnMiniMapMode) {
      return;
    }
    ...
    //   });
    // });
    let obj: IStateULBCovered | undefined | null = null;
    const stateCode = MapUtil.getStateCode(layer);
  ...
  onClickingState(
    ...
    if (this.isMapOnMiniMapMode) {
      ...
      this.initializeNationalLevelMapLayer(this.stateLayers);
      if (!skipOndropDownSelect) return;
    }
  ...
  showStateLayerOnlyFor(map: L.Map | null, state: IStateULBCovered | null) {
    if (!map || !state) return;
    map.eachLayer((layer: any) => {
  ...
  createStateLevelMap(
    ...
    this.allUlb = JSON.parse(localStorage.getItem("ulbList") as string).data[
      stateFound.code
    ].ulbs;
  ...
  onDistrictMarkerClick = (values: any, marker: L.Marker) => {
  ...
  newDashboardstateColorStyle(feature: any) {
    return {
      fillColor: "#3e5db1",
      weight: 1,
      opacity: 1,
  ...
  resetCurrentSelectState() {
    ...
    this.stateSelected.emit(undefined);
  }
