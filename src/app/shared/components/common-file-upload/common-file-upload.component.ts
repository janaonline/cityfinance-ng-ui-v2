...
export class CommonFileUploadComponent implements OnInit {
  ...
  @Input()
  dataFromParentN!: {
    pdf: { file: null; url: null; name: null; error: null; progress: null };
  ...
  ngOnInit(): void {
    ...
    if (this.quesType == 'input' && (!this.amountObj?.value || this.amountObj?.value != null)) {
  ...
  uploadFile(file: File, name: string, type: string, fileType: string | number) {
    ...
    this.dataEntryService.newGetURLForFileUpload(name, type, folderName).subscribe(
      (s3Response) => {
        ...
        this.uploadFileToS3(file, res['url'], res['path'], name, fileType);
      },
      (err) => {
        console.log(err);
