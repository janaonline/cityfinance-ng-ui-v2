...
import { RawRow } from '../afs-table/afs-table.component';
...
export class DigitizationModalComponent implements OnInit {
  ...
  async prepareFiles() {
    ...
    for (const row of this.data.selectedRows) {
      ...
      const fileData = {
        ...
        doctType: this.filters.docType,
        doctTypeLabel: row.docType,
        auditType: row.auditType,
      }
      this.seletedFiles.push({ fileUrl: ulbFile, sourceType: 'ULB', ...fileData });
      if (afsFile) this.seletedFiles.push({ fileUrl: afsFile, sourceType: 'AFS', ...fileData });
      // seletedFiles.push({
      //   ulbFile,
      //   afsFile
      // });
  ...
  private async processFileRow(fileRow: FileData): Promise<void> {
    ...
    const digitizeResp = await this.callDigitizationApi(fileRow.fileUrl);
 ...
 */
  private async handleDigitizationResponse(
    ...
    if (hasRequestId) {
      const metaBody = {
        ...
        failedSource: fileRow.sourceType,
        confidenceScore
      };

      console.log(`💾 Saving failed ${fileRow.sourceType} requestId:`, metaBody);

      await this.afsService.saveDigitizeReq(metaBody)

      console.log(`✅ Saved failed ${fileRow.sourceType} requestId for ${fileRow['ulbId']}`);
    }
  ...
  async proceedDigitization(): Promise<void> {
    ...
    for (const fileRow of this.seletedFiles) {
      try {
