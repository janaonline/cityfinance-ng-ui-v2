...
import { RolesTeamsConfig, SubRole, SUBROLE_LABEL, TeamMember } from './roles-teams-overview.models';
...
/** Shared by STATE and MoHUA — the two "Team & Roles" pages are identical apart from a handful
 *  of API endpoint paths, the default subRole fallback, and some copy text, all supplied via
 *  route `data.rolesConfig` (see roles-teams-overview.models.ts and each module's routes file). */
@Component({
...
export class RolesTeamsOverviewComponent implements OnInit {
  ...
  private readonly config: RolesTeamsConfig = this.route.snapshot.data['rolesConfig'];

  readonly permissionMatrix = signal<PermissionRow[]>([]);
  readonly pageSubtitle = this.config.pageSubtitle;
  readonly tableAriaLabel = this.config.tableAriaLabel;
  ...
  readonly infoBannerText = this.config.infoBannerText;

  private currentUserId = '';

  readonly currentSubRole = signal<SubRole>('VIEWER');
  readonly currentUserName = signal<string>('');

  getRoleLabel(subRole: string): string {
    return SUBROLE_LABEL[subRole as SubRole] ?? subRole;
  }
  ...
  readonly addForm = this.fb.nonNullable.group({
    ...
    subRole: ['EDITOR' as SubRole, Validators.required],
  });

  ngOnInit(): void {
    const u = this.profileService.readStoredUser();
    this.currentUserId = u._id ?? u.id ?? '';
    this.currentSubRole.set((u['subRole'] as SubRole | undefined) ?? this.config.defaultSubRole);
  ...
  private loadPermissionMatrix(): void {
    this.http
      .get<{ success: boolean; data: PermissionRow[] } | PermissionRow[]>(`${this.baseUrl}${this.config.permissionMatrixPath}`)
      .pipe(
        map((r): PermissionRow[] =>
  ...
  loadMembers(): void {
    ...
    this.http
      .get<{ success: boolean; data: TeamMember[] } | TeamMember[]>(`${this.baseUrl}${this.config.membersListPath}`)
      .pipe(
        // Dev server returns raw array; local NestJS ResponseTransformInterceptor wraps { success, data }
        map((r): TeamMember[] =>
          'success' in r && Array.isArray((r as { data: unknown }).data)
            ? (r as { success: boolean; data: TeamMember[] }).data
            : (r as TeamMember[]),
        ),
        catchError(() => of(null)),
        takeUntilDestroyed(this.destroyRef),
  ...
  private submitInvite(action: 'invite' | 'restore'): void {
    ...
    this.http
      .post<unknown>(`${this.baseUrl}${this.config.invitePath}`, payload)
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          ...
          if (body['code'] === 'EMAIL_XVIFC_REMOVED') {
            ...
            const errors = body['errors'] as Record<string, { message: string }[]> | undefined;
            const emailError = errors?.['email']?.[0];
            if (emailError) {
              // Shown inline under the Email field (see getFirstError) — no need to also
              // repeat it in the generic banner below the form.
              this.addForm.controls.email.setErrors({ domainMx: true });
  ...
  changeRole(member: TeamMember, newRole: SubRole): void {
    ...
    this.http
      .patch(`${this.baseUrl}${this.config.memberActionBasePath}/${member._id}/sub-role`, { subRole: newRole })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
  ...
  confirmDelete(member: TeamMember): void {
    ...
    this.http
      .delete(`${this.baseUrl}${this.config.memberActionBasePath}/${member._id}`)
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
  ...
  confirmTransfer(): void {
    ...
    this.http
      .post(`${this.baseUrl}${this.config.transferSubmitterPath}`, { toUserId })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: () => {
  ...
  trackByMemberId(_: number, m: TeamMember): string {
    return m._id;
  }
}
