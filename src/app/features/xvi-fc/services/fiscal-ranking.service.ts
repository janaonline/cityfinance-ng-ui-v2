...
export const removeFalsy = (obj: { [s: string]: unknown; } | ArrayLike<unknown>) => Object.entries(obj).reduce((a: any, [k, v]) => (v ? (a[k] = v, a) : a), {});

const getValueByPath = (response: any, path: string) => {
  return path.split('.').reduce((value: { [x: string]: any; }, key: string | number) => (value && value[key] !== undefined ? value[key] : undefined), response);
}
...
export const tableMapperPipe = (columns: any, tablePath: string = '') => {
  return map((response: any) => {
    ...
    mutable.columns = columns || mutable.columns.map((column: any[]) => ({
      ...column,
      sort: column.sort || 0,
    }));
    return response;
...
export class FiscalRankingService {
  ...
  getTableResponse(endpoint: string, queryParams: string, columns: any, tablePath: string = '', params = {}) {
